...
void CLI_Receive(uint8_t *input, uint16_t size) {
		...
		{            
			...
			else if (strcmp(inputString, (const char *)"f1") == 0)
			{
				const char frequency_stmt[] = "\r\nChanging frequency to 500 ms"; 
				CLI_Transmit(frequency_stmt, sizeof(frequency_stmt));
				xQueueSendToFront(freqQueue, (void*)500, 0);
			}
			else if (strcmp(inputString, (const char *)"f2") == 0)
			{
				const char frequency_stmt[] = "\r\nChanging frequency to 1000 ms"; 
				CLI_Transmit(frequency_stmt, sizeof(frequency_stmt));
				xQueueSendToFront(freqQueue, (void*)1000, 0);

			}
			else if (strcmp(inputString, (const char *)"f3") == 0)
			{
				const char frequency_stmt[] = "\r\nChanging frequency to 1500 ms"; 
				CLI_Transmit(frequency_stmt, sizeof(frequency_stmt));
				xQueueSendToFront(freqQueue, (void*)1500, 0);
